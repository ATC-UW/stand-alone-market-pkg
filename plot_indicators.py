"""Plot all technical indicators for a simulated market."""
...
days = list(range(len(md.getBuyPrices())))
buy = md.getBuyPrices()
sell = md.getSellPrices()

# Fetch indicators
sma_20 = md.getBuySMA(period=20)
ema_20 = md.getBuyEMA(period=20)
rsi_14 = md.getBuyRSI(period=14)
macd_line, signal_line, histogram = md.getBuyMACD()
bb_upper, bb_middle, bb_lower = md.getBuyBollingerBands(period=20)
atr_14 = md.getATR(period=14)

fig, axes = plt.subplots(5, 1, figsize=(14, 16), sharex=True)
fig.suptitle("Technical Indicators Dashboard", fontsize=14, fontweight="bold")

# --- 1. Price + SMA/EMA ---
ax = axes[0]
ax.plot(days, buy, label="Buy Price", color="steelblue", linewidth=1)
ax.plot(days, sell, label="Sell Price", color="lightcoral", linewidth=0.8, alpha=0.6)
...
ax.bar(days, histogram, color=colors, alpha=0.4, width=1.0, label="Histogram")
